...
import { addItem } from "./CartSlice"; // Importing addItem from CartSlice

function ProductList() {
  const [showCart, setShowCart] = useState(false);
  const [addedToCart, setAddedToCart] = useState({});
  const dispatch = useDispatch();
  const cart = useSelector((state) => state.cart.items); // Retrieve cart items from Redux store
  ...
  const handleAddToCart = (product) => {
    dispatch(addItem(product)); // Dispatch addItem action to add product to cart
    setAddedToCart((prevState) => ({
      ...prevState,
      [product.name]: true, // Mark product as added to cart
    }));
  };
